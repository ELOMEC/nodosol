...
use crate::{
    constants::{
        ...
        TREE_CANOPY_DEPTH, TREE_MAX_BUFFER_SIZE, TREE_MAX_DEPTH,
    },
...
pub fn handle_initialize_event_tree(ctx: Context<InitializeEventTree>) -> Result<()> {
    ...
    // Devnet/mainnet Bubblegum dispatches on the legacy "create_tree" name
    // (post-Kinobi "create_tree_config" returns InstructionFallbackNotFound
    // on the currently deployed program). sha256("global:create_tree")[..8]
    let discriminator: [u8; 8] = [165, 83, 136, 142, 89, 202, 47, 220];
