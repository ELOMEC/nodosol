...
/// Permissionless cleanup: anyone can reclaim an expired listing — the
/// cNFT goes back to the seller, and the PDA rent is refunded to the
/// seller too. Seller doesn't need to be online. This keeps orphaned
/// listings from piling up indefinitely.
#[derive(Accounts)]
pub struct CloseExpiredResale<'info> {
    /// Anyone pays the compute; they don't need to be seller.
    #[account(mut)]
    pub caller: Signer<'info>,

    /// CHECK: receives the cNFT + rent refund. Must match listing.seller.
...
pub fn handle_close_expired_resale<'info>(
...

    {
        ...
        require!(listing.expires_at > 0, EventTicketsError::ResaleNotExpired);
        require!(now >= listing.expires_at, EventTicketsError::ResaleNotExpired);
    }
    ...
    // `close = seller` on the listing account refunds rent to the seller.
    Ok(())
}
