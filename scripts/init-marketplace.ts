/**
 * Devnet init for the marketplace program.
 *
 * - Initialises Config PDA with fee_bps = 250 (2.5%), treasury = dev
 *   wallet's mock-USDC ATA.
 * Idempotent: skips if Config PDA already exists.
 *
 * Run:  npm run init-marketplace
 */
...
async function main() {
  ...
  const program = new Program(marketplaceIdl as never, provider);
  const programId = new PublicKey((marketplaceIdl as { address: string }).address);
  console.log("marketplace program:", programId.toBase58());

  // Treasury = dev wallet's mock-USDC ATA (from earlier init-demo run).
  ...
  console.log("Marketplace config PDA:", configPda.toBase58());

  const existing = await connection.getAccountInfo(configPda);
  if (existing) {
    console.log("Marketplace config already initialised — skipping");
  } else {
