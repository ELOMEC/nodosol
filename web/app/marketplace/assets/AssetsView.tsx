...
export function AssetsView() {
  ...
  return (
    <>
      <header style={HERO}>
        <div>
          <span style={EYEBROW}>Issuer workspace</span>
          <h1 style={H1}>
            My assets
          </h1>
          <p style={SUB}>
            Assets you have tokenised as an issuer. Issuer wallet:{" "}
            <span style={{ color: "var(--shell-fg)", fontWeight: 800 }}>{publicKey && shorten(publicKey.toBase58())}</span>
          </p>
        </div>
        <Link
  ...
  return (
    <>
      <header style={HERO}>
        ...
        <p style={SUB}>Assets you have tokenised as an issuer.</p>
        </div>
      </header>
      <div
        style={{
...
function StatCard({ label, value, sub, valueColor }: { label: string; value: string; sub: string; valueColor?: string }) {
  return (
    <div style={{ background: "var(--shell-card)", border: "1px solid var(--shell-border)", borderRadius: 16, padding: "1.05rem 1.1rem", boxShadow: "0 14px 36px rgba(15, 23, 42, 0.05)" }}>
      ...
      <div style={{ fontSize: "1.55rem", fontWeight: 800, letterSpacing: 0, color: valueColor ?? "var(--shell-fg)" }}>{value}</div>
