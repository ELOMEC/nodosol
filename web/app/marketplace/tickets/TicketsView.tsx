...
export function TicketsView() {
  ...
  return (
    <>
      <header style={HERO}>
        <div>
        <span style={EYEBROW}>Wallet tickets</span>
        <h1 style={H1}>
          My tickets
        </h1>
        <p style={SUB}>
...
function TicketCard({ ticket }: { ticket: TicketRow }) {
  ...
  return (
    ...
    >
    <div style={{ background: "var(--shell-card)", border: "1px solid var(--shell-border)", borderRadius: 16, overflow: "hidden", boxShadow: "0 16px 45px rgba(15, 23, 42, 0.06)" }}>
      <div
        style={{
          height: 140,
      ...
      <div style={{ padding: "0.95rem 1.05rem 1.05rem" }}>
        <div style={{ fontSize: "0.95rem", fontWeight: 800, marginBottom: "0.15rem" }}>{ticket.name}</div>
        {ticket.event ? (
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "0.4rem" }}>
          ...
          <a
            ...
            style={{ fontSize: "0.75rem", color: "#0369a1", fontWeight: 800, textDecoration: "none" }}
          >
            Explorer ↗
          </a>
        </div>
  ...
  return (
    <div style={{ background: "var(--shell-card)", border: "1px solid var(--shell-border)", borderRadius: 16, overflow: "hidden", boxShadow: "0 16px 45px rgba(15, 23, 42, 0.06)" }}>
      ...
      <div style={{ padding: "0.85rem 1rem 0.95rem" }}>
        <div style={{ fontSize: "0.88rem", fontWeight: 800, marginBottom: "0.2rem" }}>{name}</div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "0.4rem" }}>
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            style={{ fontSize: "0.72rem", color: "#0369a1", fontWeight: 800, textDecoration: "none" }}
          >
            Explorer ↗
          </a>
        </div>
...
function StatCard({ label, value, sub }: { label: string; value: string; sub: string }) {
  return (
    <div style={{ background: "var(--shell-card)", border: "1px solid var(--shell-border)", borderRadius: 16, padding: "1.05rem 1.1rem", boxShadow: "0 14px 36px rgba(15, 23, 42, 0.05)" }}>
      <div style={{ fontSize: "0.72rem", color: "var(--shell-muted)", marginBottom: "0.5rem", fontWeight: 800, letterSpacing: "0.08em", textTransform: "uppercase" }}>{label}</div>
      <div style={{ fontSize: "1.55rem", fontWeight: 800, letterSpacing: 0, color: "var(--shell-fg)" }}>{value}</div>
      <div style={{ fontSize: "0.76rem", color: "var(--shell-muted)", marginTop: "0.25rem" }}>{sub}</div>
    </div>
  );
}
...
function CenteredCard({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ background: "var(--shell-card)", border: "1px solid var(--shell-border)", borderRadius: 16, padding: "3rem 1.5rem", textAlign: "center", color: "var(--shell-muted)", boxShadow: "0 16px 45px rgba(15, 23, 42, 0.06)" }}>
      {children}
    </div>
  );
}
